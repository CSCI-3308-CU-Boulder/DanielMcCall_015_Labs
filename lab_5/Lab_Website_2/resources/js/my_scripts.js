...
function loadStatsPage() {
	...
	for (row = 2; row < table.rows.length; row++) {
		...
		else if (parseInt(table.rows[row].cells[2].innerHTML) < parseInt(table.rows[row].cells[3].innerHTML)) {
			table.rows[row].cells[4].innerHTML = table.rows[row].cells[1].innerHTML;
		}
	}

	document.getElementById("wins").innerHTML = wins;
	document.getElementById("losses").innerHTML = table.rows.length - 2 - wins;
...
function loadPlayersPage() {
	...
	for (i = 0; i < players.length; i++) {
		...
		dropdown.append(document.createElement("br"));
...
function switchPlayers(playerNum) {
	...
	var avgPassingYards = players[playerNum].pass_yards/players[playerNum].games_played;
	...
	document.getElementById("avg_p_yards").innerHTML = avgPassingYards;
