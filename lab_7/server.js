...
app.get('/player_info/player', function(req, res) {
	...
	var query3 = 'SELECT COUNT(*) FROM football_games WHERE players[1] = \'' + player_id + '\' OR players[2] = \'' + player_id + '\' OR players[3] = \'' + player_id + '\' OR players[4] = \'' + player_id + '\' OR players[5] = \'' + player_id + '\';';
	db.task('get-everything', task => {
    	return task.batch([
        	task.any(query1),
        	task.any(query2),
...
app.post('/player_info/add_player', function(req, res) {
	...
	var insert_statement = 'INSERT INTO football_players(name, year, major, passing_yards, rushing_yards, receiving_yards, img_src) VALUES(\'' + player_name +'\', \'' + player_year + '\', \'' + player_major + '\', \'' + player_passing_yards + '\', \'' + player_rushing_yards + '\', \'' + player_receiving_yards + '\', \'' + "../resources/img/helmet.jpg" + '\');'; 
	var player_select = 'SELECT * FROM football_players;';
...
app.post('/player_info/add_game', function(req, res) {
	...
	var game_select = 'SELECT * FROM football_games;';
	var query2 = 'SELECT COUNT(*) FROM football_games WHERE home_score > visitor_score;';
	var query3 = 'SELECT COUNT(*) FROM football_games WHERE home_score < visitor_score;';
